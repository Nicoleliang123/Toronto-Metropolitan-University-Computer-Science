...
def digitcat(s):
    ...
    #If result is empty set, return None
    #NOT s because s will still contain a string
    #This must happen after validating no digit is in the str
    if not result:
        return None
...
def stringtofloatlist(fltstr):
    ...
    result = [float(x) for x in fltstr]
