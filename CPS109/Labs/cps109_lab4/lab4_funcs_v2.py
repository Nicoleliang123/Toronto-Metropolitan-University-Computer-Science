...
def sumeven(n):
    ...
    #Initialize even numbers
    even = 0
    
    #Keep looping as long not negative
    while n >= 0:
        
        #iterate to the range of n number
        for i in range(n):
            
            #keep multiplying to even numbers and add onto evensum
            even += i*2
...
def sumsquares(n):
    ...
    while n >= 0:
        ...
        #Iterate starting from 1 assuming as the first square, and n+1
        #to be inclusive
        for i in range(1, n+1):
            
            #Multiply itself and add onto squares
            #Be careful not exponential 
            squares += (i*i)
...
def odddigitsum(num):
    ...
    #get absolute value to avoid negatives
    num = abs(num)
    
    #initialize odd_sum
    odd_sum = 0
    
    #if number is empty or is zero
    if len(str(num)) == 0 or num == 0:
        return 0
    
    #while num is always bigger or equal to one
    while num >= 1:
        
        #iterate till however long digits there are in num
        #str(num) to use len()
        for i in range(len(str(num))):
            
            #Conversion of num into a string 
            #For later access of getting an index
            s = str(num)
            
            #Get the index of the string
            #Turn the index str into an int 
            #For further calculations in math
            digit = int(s[i])
            
            #If that index is not divisible by 2,
            #Odd_digit, add to sum
            if digit % 2 != 0:
...
def listexponential(n, base):
    ...
    for i in range(n):
        ...
        #keep adding numbers into the list of the calculation of:
        #base exponent to i value that keeps increasing
...
def digitcat(s):
    ...
    #Create an new result empty string which later converted to integer
    result = ""
    
    #For every character in s, checks every scenario
    for character in s:
        
        #If character is an digit
        if character.isdigit():
            
            #Add the character value into the result string
            result += character
    ...
    #Otherwise, return the integer of result string
    return int(result)
...
def stringtofloatlist(fltstr):
    ...
    #If fltstr is an empty string
    if not fltstr:
        
        #Return an empty list
        return []
    
    #Split fltstr string with commas
    fltstr = fltstr.split(",")
    
    #Create an result empty list
    result = []
    
    #Convert every x as float for every x element in fltstr
    fltstr = [float(x) for x in fltstr]
    
    #Add fltstr into result list
    result += fltstr
...
def maxbytype(items):
    ...
    #for x in items:
        #if type(x) == int:
            #code block -- 
    largest_int = max([x for x in items if type(x) == int], default = None)
    
    largest_float = max([x for x in items if type(x) == float], default = None)
    
    largest_string = max([x for x in items if type(x) == str], default = None)
    
    return(largest_int, largest_float, largest_string)
