...
def two_summers(items, target):
    ...
    left, right = 0, len(items) - 1  # Start  two pointers
...
def count_dominators(items):
    ...
    for value in reversed(items):
        if value > max_right:
            count += 1
            max_right = value  # Update new max
    return count
...
def safe_squares_rooks(n, rooks):
    ...
    # One pass through the rooks list
    for r, c in rooks:
        attacked_rows.add(r)
        attacked_cols.add(c)

    # Count of safe rows and columns
    safe_rows = n - len(attacked_rows)
    safe_cols = n - len(attacked_cols)

    # Each safe square is in a safe row AND a safe column
...
def remove_after_kth(items, k):
    ...
    for item in items:
        # Get how many times we've seen this item (default 0)
        if counts.get(item, 0) < k:
            result.append(item)
