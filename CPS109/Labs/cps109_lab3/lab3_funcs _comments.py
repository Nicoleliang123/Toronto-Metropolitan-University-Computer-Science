...
def duplicates(items):
    ...
    #If items don't contain exactly 3 elements 
    if len(items) != 3:
        return "invalid input"
    
    #Create variable names for eaach elements in the items list
    a, b, c = items
...
def inversions(items):
    ...
    #If length of items list is not exactly 3 elements long
    if len(items) != 3:
        return -1 
    
    #Create variables for elements in the list
    a,b,c = items
...
def increasing(items, strict):   
    ...
    #If strict is not in T/F
    if strict not in (True, False):
        return 'invalid input'
    
    a,b,c = items
    
    #In the case strict is true, check strictly ascending
    if strict == True:
    ...
    #Otherwise only regular ascending
    else:
...
def calculator(op1, op2, operator):     
    ...
    #If operator given is not in one of these
    if operator not in ("+", "-", "*", "/", "**"):
        return None
